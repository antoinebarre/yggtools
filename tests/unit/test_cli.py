...
from uvforge.cli import app
from uvforge.init import ConflictError
...
def test_init_dry_run_exits_zero() -> None:
    """Requirement: uvforge init --dry-run must exit 0 without files."""
    with patch("uvforge.cli.run_init"):
        result = _runner.invoke(app, ["init", "mylib", "--dry-run"])
    assert result.exit_code == 0


def test_init_passes_project_name_to_run_init() -> None:
    """Requirement: uvforge init must forward the project name to run_init."""
    with patch("uvforge.cli.run_init") as mock_init:
...
def test_init_defaults_python_version() -> None:
    """Requirement: uvforge init must default to Python 3.12."""
    with patch("uvforge.cli.run_init") as mock_init:
...
def test_init_respects_custom_python() -> None:
    """Requirement: uvforge init --python must set python_version."""
    with patch("uvforge.cli.run_init") as mock_init:
...
def test_init_no_git_flag() -> None:
    """Requirement: uvforge init --no-git must set no_git=True on context."""
    with patch("uvforge.cli.run_init") as mock_init:
...
def test_init_force_flag() -> None:
    """Requirement: uvforge init --force must set force=True on context."""
    with patch("uvforge.cli.run_init") as mock_init:
...
def test_init_exits_1_on_uv_not_found() -> None:
    """Requirement: uvforge init must exit 1 when uv is not found."""
    with patch("uvforge.cli.run_init", side_effect=UvNotFoundError("no uv")):
        result = _runner.invoke(app, ["init", "mylib"])
    assert result.exit_code == 1


def test_init_exits_1_on_conflict_error() -> None:
    """Requirement: uvforge init must exit 1 on ConflictError."""
    with patch("uvforge.cli.run_init", side_effect=ConflictError("conflict")):
        result = _runner.invoke(app, ["init", "mylib"])
    assert result.exit_code == 1


def test_init_exits_1_on_unexpected_error() -> None:
    """Requirement: uvforge init must exit 1 on any unexpected exception."""
    with patch("uvforge.cli.run_init", side_effect=RuntimeError("boom")):
        result = _runner.invoke(app, ["init", "mylib"])
    assert result.exit_code == 1


def test_check_exits_0_on_all_pass(tmp_path: Path) -> None:
    """Requirement: uvforge check must exit 0 when all checks pass."""
    all_pass = [CheckResult(label="src/", passed=True)]
    with patch("uvforge.cli.run_check", return_value=all_pass):
        result = _runner.invoke(app, ["check", str(tmp_path)])
    assert result.exit_code == 0


def test_check_exits_1_on_failure(tmp_path: Path) -> None:
    """Requirement: uvforge check must exit 1 when at least one check fails."""
    one_fail = [CheckResult(label="src/", passed=False, detail="missing")]
    with patch("uvforge.cli.run_check", return_value=one_fail):
        result = _runner.invoke(app, ["check", str(tmp_path)])
    assert result.exit_code == 1
